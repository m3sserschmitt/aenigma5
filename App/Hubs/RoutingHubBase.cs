    ...
    protected async Task SendAsync(string connectionId, byte[] data)
    {
        try
        {
            var routingMethod = typeof(TSelf).GetMethods()
                .Where(m => m.GetCustomAttribute<OnionRouterAttribute>() != null)
                .SingleOrDefault();
        ...
        {
            throw new Exception($"Type {nameof(TSelf)} should contain exactly one method with {nameof(OnionRouterAttribute)}.");
        }
    }
}
